"""
Weighted Top Five Portfolio Selection Module
...
2. Allocates weights as 40%, 25%, 20%, 10%, 5% to the top 5 performers
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("weighted_top_five.log"),
    ],
)
...
def calculate_weights(selected_assets: List[str]) -> pd.Series:
    """
    Calculate portfolio weights for selected assets.
    Top performer gets 40%, next gets 25%, then 20%, 10%, and 5%.
    ...
    weight_distribution = [0.40, 0.25, 0.20, 0.10, 0.05]

    # Create weights series
    weights = pd.Series(weight_distribution[:n_assets], index=selected_assets)

    return weights


def create_weighted_top_five_portfolio(
    returns: pd.DataFrame,
    rebalance_frequency: str = "Q",
    lookback_window: int = 63,
) -> Tuple[pd.Series, Dict[str, Any]]:
    """
    Create and manage a weighted top five portfolio.
    ...
    for i, date in enumerate(returns.index[1:], 1):
        ...
        if date in rebalance_dates:
            ...
            current_weights = calculate_weights(selected_assets)
...
def main(
    ...
    """
    Main function to run the weighted top five portfolio strategy.
    ...
    # Create portfolio
    portfolio_returns, portfolio_info = create_weighted_top_five_portfolio(
        returns,
    ...
    output_file = os.path.join(output_dir, "weighted_top_five_portfolio.csv")
    portfolio_returns.to_csv(output_file)
    logging.info(f"Saved portfolio returns to {output_file}")

    # Save portfolio info
    info_file = os.path.join(output_dir, "weighted_top_five_info.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run weighted top five portfolio selection strategy"
    )
    parser.add_argument(

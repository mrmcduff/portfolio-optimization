...
def load_data(
    weights_file: str, returns_file: str
...
def plot_efficient_frontier(
    ...
    min_weights, min_vol, min_ret = min_variance_portfolio(
        returns, cov_matrix, long_only
    )
    ...
    for target_ret in target_returns:
        try:
            weights, vol, ret = optimize_portfolio(
...
def plot_portfolio_weights(
    weights: pd.Series, output_file: Optional[str] = None
) -> plt.Figure:
    """
...
def plot_rolling_returns_histogram(
    returns: pd.Series, window: int = 252, output_file: Optional[str] = None
) -> plt.Figure:
    """
...
def main(
    ...
    if not benchmark_returns.empty:
        ...
        plot_benchmark_comparison(
            portfolio_returns, benchmark_returns, output_file=comparison_file
        )

        # 5. Plot risk metrics comparison
        metrics_file = os.path.join(output_dir, "risk_metrics.png")
        plot_risk_metrics(
            performance_metrics, benchmark_metrics, output_file=metrics_file
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights", "-w", required=True, help="Path to the weights CSV file"
    )
    parser.add_argument(
